...
-- -----------------------------------------------------------------------------
-- 6. Daily Tasks (13 rows) — 5 inactive (share and special tasks), 8 active
-- -----------------------------------------------------------------------------
INSERT INTO daily_tasks (id, name, description, kind, requirement, metadata, is_active, rule_id, convention_id) VALUES
  ('e9c1d39b-1cf4-4a93-80f5-d428dd286560', 'Open the leaderboard',          'See who is leading the pack right now.',                   'leaderboard', 1,  '{"metric":"total","eventType":"leaderboard_refreshed"}',                                                               true,  null, null),
  ('918258d9-651d-4e24-9c61-b4703ce4852e', 'Catch 1 suiter today',          'Kick off the day by catching any suiter.',                 'catch',       1,  '{"metric":"total","eventType":"catch_performed"}',                                                                     true,  null, null),
  ('26139b89-a341-454a-bba8-3ff38c2f51ac', 'Catch 3 suiters today',         'Keep the momentum going with three total catches.',         'catch',       3,  '{"metric":"total","eventType":"catch_performed"}',                                                                     true,  null, null),
  ('86765af1-77e8-4ca0-a3dc-51531dc9c6c2', 'Catch 10 suiters today',        'Go on a catching spree by snagging ten suiters today.',     'catch',       10, '{"metric":"total","eventType":"catch_performed"}',                                                                     true,  null, null),
  ('85a59e7b-6906-4b84-a133-95a25a1f2937', 'Catch 5 unique suiters',        'Track down five different suiters in a single day.',         'catch',       5,  '{"metric":"unique","uniqueBy":"payload.fursuit_id","eventType":"catch_performed"}',                                    true,  null, null),
  ('c5914eb9-dbd2-41b0-8176-b98199e1eccf', 'Refresh the leaderboard twice', 'Check back in on the leaderboard a second time.',           'leaderboard', 2,  '{"metric":"total","eventType":"leaderboard_refreshed"}',                                                               true,  null, null),
  ...
  ('190aa081-2120-4a89-8b5a-0ee0cb576ce2', 'View 1 suiter bio',             'Open any suiter bio to learn more about them.',              'view_bio',    1,  '{"metric":"total","filters":[{"path":"payload.owner_id","notEqualsUserId":true}],"eventType":"fursuit_bio_viewed"}',    true,  null, null),
  ('61334de8-f453-488f-9a8f-35ded68483e3', 'View 3 suiter bios',            'Check out bios for three different suiters.',                'view_bio',    3,  '{"metric":"unique","filters":[{"path":"payload.owner_id","notEqualsUserId":true}],"uniqueBy":"payload.fursuit_id","eventType":"fursuit_bio_viewed"}', true, null, null),
  ('8b5f9a7a-8d7d-4e89-9a92-4fd4b45b8b71', 'Same Studio',                  'Catch a suit from a maker that also made one of yours.',     'catch',       1,  '{"metric":"total","eventType":"catch_performed","filters":[{"path":"payload.catcher_id","equalsUserId":true},{"path":"payload.has_catcher_owned_maker_match","equals":true}],"defaultRotationEligible":false,"rotationPool":"special"}', false, null, null),
  ('3327bbbe-2c41-421d-ad0b-ed1c3fb22d60', 'Studio Sampler',               'Catch suits from two different makers today.',               'catch',       2,  '{"metric":"unique","uniqueBy":"payload.normalized_maker_names","eventType":"catch_performed","filters":[{"path":"payload.catcher_id","equalsUserId":true},{"path":"payload.has_maker","equals":true}],"defaultRotationEligible":false,"rotationPool":"special"}', false, null, null),
  ('ee1c9130-0db4-48cf-bbb6-64f965a563c7', 'Fresh Workshop',               'Catch a maker you have not caught earlier at this convention.', 'catch',    1,  '{"metric":"total","eventType":"catch_performed","filters":[{"path":"payload.catcher_id","equalsUserId":true},{"path":"payload.is_new_maker_for_catcher_at_convention","equals":true}],"defaultRotationEligible":false,"rotationPool":"special"}', false, null, null)
ON CONFLICT (id) DO NOTHING;
