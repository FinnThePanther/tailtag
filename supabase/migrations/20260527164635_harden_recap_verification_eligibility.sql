-- Location-required convention recaps require verified attendance, not just pre-registration.
...
WITH invalid_recap_notifications AS (
  ...
  LEFT JOIN public.conventions c
    ON c.id = cpr.convention_id
...
WITH invalid_recaps AS (
  ...
  JOIN public.conventions c ON c.id = cpr.convention_id
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.profile_conventions pc
