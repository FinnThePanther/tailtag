-- Enforce adult interaction boundaries during catch creation.
-- The Edge Function calls create_catch_with_event through service_role, so catch
-- authorization must evaluate the supplied catcher profile directly instead of
-- relying on auth.uid().

CREATE OR REPLACE FUNCTION public.can_catch_fursuit_as_profile(
  p_catcher_id uuid,
  p_fursuit_id uuid
)
RETURNS boolean
...
AS $function$
  SELECT EXISTS (
    ...
    JOIN public.profiles catcher_profile ON catcher_profile.id = p_catcher_id
    WHERE f.id = p_fursuit_id
      ...
      AND (
        ...
        OR (
          ...
          AND coalesce(catcher_profile.is_adult, false) = true
        )
      )
  );
$function$;

REVOKE ALL ON FUNCTION public.can_catch_fursuit_as_profile(uuid, uuid)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.can_catch_fursuit_as_profile(uuid, uuid)
  TO service_role;
