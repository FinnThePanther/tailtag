-- Eliminate pg_timezone_names catalog scan in is_convention_prejoinable.
-- The catalog lookup is unnecessary — timezone() validates internally
-- and throws on invalid names. This matches the simpler pattern already
-- used by is_convention_joinable (20260505120000_prejoin_conventions.sql).
...
AS $$
  SELECT COALESCE((
    ...
    CROSS JOIN LATERAL (
      SELECT timezone(COALESCE(NULLIF(btrim(c.timezone), ''), 'UTC'), now())::date AS local_day
    ) info
    WHERE c.id = p_convention_id
  ), false);
$$;

-- Inline the distance calculation in verify_and_opt_in_to_convention
-- to avoid the redundant auth/profile/convention lookups and double
-- verification_attempts INSERT triggered by the nested call to
