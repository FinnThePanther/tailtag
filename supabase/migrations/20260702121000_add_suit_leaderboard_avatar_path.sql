-- Return fursuit avatar storage paths for suit leaderboard rows so clients can
-- resolve authenticated Storage URLs even when legacy avatar_url is absent.
...
AS $function$
  ...
  SELECT
    ...
    CASE WHEN public.can_view_fursuit(cu.id, l.fursuit_id) THEN f.avatar_path ELSE NULL END AS fursuit_avatar_path,
