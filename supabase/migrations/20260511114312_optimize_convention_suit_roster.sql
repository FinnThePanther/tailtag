...
REVOKE ALL ON FUNCTION public.get_convention_suit_roster_caught_ids(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_convention_suit_roster_caught_ids(uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_convention_suit_roster_caught_ids(uuid) TO authenticated;

CREATE INDEX IF NOT EXISTS idx_catches_convention_fursuit_accepted
  ON public.catches (convention_id, fursuit_id)
  WHERE status = 'ACCEPTED' AND is_tutorial = false;

CREATE INDEX IF NOT EXISTS idx_catches_convention_catcher_accepted
  ON public.catches (convention_id, catcher_id, fursuit_id)
  WHERE status = 'ACCEPTED' AND is_tutorial = false;
