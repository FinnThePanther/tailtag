...
2. **OTA updates:** Pushes to `dev` auto-publish to the `staging` EAS Update channel; pushes to `main` auto-publish to `production`. The CI splits mobile changes two ways: **JS-only** pushes (`app/`, `src/`, `assets/`, `packages/`, `package.json`, `package-lock.json`) ship via OTA alone — no native rebuild, testers don't re-install. This covers adding pure-JS deps like `lodash`. **Native** pushes (`ios/`, `android/`, `app.config.ts`, `eas.json`) trigger both a full EAS Build and an OTA; `ios/`/`android/` are the authoritative signal because any real native-module install must be accompanied by `expo prebuild` that updates those dirs. Runtime version uses the `appVersion` policy, so **native pushes MUST include a `package.json` version bump** — the `guard_version_bump` job fails the workflow otherwise, preventing OTAs that would crash older binaries missing the new native modules. Add `[skip eas]` to the merge commit to opt out of both paths (and the guard).
