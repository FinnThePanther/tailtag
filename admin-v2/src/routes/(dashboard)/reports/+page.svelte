    ...
    <Table headers={['Type', 'Status', 'Reporter', 'Target', 'Description', 'Created', 'Actions']}>
      {#each data.reports as report}
        ...
        <tr>
          ...
          <td class="px-4 py-3 text-slate-200">{reporter ?? report.reporter_id}</td>
          <td class="px-4 py-3 text-slate-200">
            <div class="space-y-2">
              {#if report.reported_user_id}<div><span class="mr-2 text-xs text-muted">User</span><a href={`/players/${report.reported_user_id}`} class="rounded-lg border border-border px-2.5 py-1 text-xs font-semibold text-slate-100 transition hover:border-primary">{target ?? report.reported_user_id}</a></div>{/if}
              {#if report.reported_fursuit_id}<div class="space-y-1 text-sm"><div><span class="text-xs text-muted">Fursuit</span> <span>{reportedFursuit?.name ?? report.reported_fursuit_id}</span></div>{#if reportedFursuit?.owner_id}<div><span class="text-xs text-muted">Owner</span> <a href={`/players/${reportedFursuit.owner_id}`} class="text-xs font-semibold text-slate-100 underline decoration-border underline-offset-4 transition hover:text-primary">{reportedFursuit.owner?.username ?? reportedFursuit.owner_id}</a></div>{/if}</div>{/if}
              {#if !report.reported_user_id && !report.reported_fursuit_id}<span class="text-muted">-</span>{/if}
            </div>
          </td>
          <td class="px-4 py-3 text-slate-200">{#if report.description}<span class="line-clamp-2 max-w-xs text-sm">{report.description}</span>{:else}<span class="text-muted">-</span>{/if}</td>
          <td class="px-4 py-3 text-slate-200">{report.created_at ? new Date(report.created_at).toLocaleString() : '-'}</td>
          <td class="px-4 py-3">
            ...
            {:else}
              <form class="space-y-2" method="POST" action="?/resolve">
                ...
                <textarea name="resolutionNotes" placeholder="Resolution notes" class="w-48 rounded-lg border border-border bg-background px-2 py-1 text-xs text-slate-100 outline-none focus:border-primary"></textarea>
                <div class="flex gap-2">
                  <button name="status" value="resolved" class="rounded-lg bg-primary px-2.5 py-1 text-xs font-semibold text-slate-900 transition hover:bg-accent">Resolve</button>
                  <button name="status" value="dismissed" class="rounded-lg border border-border px-2.5 py-1 text-xs font-semibold text-slate-100 transition hover:border-primary">Dismiss</button>
                </div>
              </form>
            {/if}
          </td>
