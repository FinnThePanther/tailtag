...
const buildColorLine = (value: FursuitColorOption[] | undefined) => {
  ...
  const names = value.map((option) => option.name.trim()).filter((name) => name.length > 0);
...
export function FursuitCard({
  ...
  return (
    <Pressable
      ...
      disabled={!onPress}
      onPress={onPress}
      style={({ pressed }) => [styles.container, onPress ? { opacity: pressed ? 0.92 : 1 } : null]}
    >
      <View style={styles.leadRow}>
        ...
        <View style={styles.details}>
          ...
          {timelineLabel ? <Text style={styles.timeline}>{timelineLabel}</Text> : null}
        </View>
      </View>
