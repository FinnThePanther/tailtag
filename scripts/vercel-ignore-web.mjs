...
const watchedPaths = parseList(process.env.WATCHED_PATHS, ['web/']);
...
function log(message) {
  console.log(`[vercel-ignore-web] ${message}`);
}
...
function hasChangedFiles() {
  ...
  try {
    ...
    console.error('[vercel-ignore-web] Failed to inspect changed files.', error);
  }
...
if (!hasChangedFiles()) {
  log('Skipping deployment because web files did not change.');
  process.exit(0);
}

log('Web files changed on an allowed branch; building deployment.');
process.exit(1);
